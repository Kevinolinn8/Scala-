...
object Evaluator5 {
  def typecheck(env : Map[String,Type], e : Expr) : Type = {
    e match {
      ...
      /* TODO */
      case BopExpr(e1,OrBop,e2) => BoolType 
      case BopExpr(e1,PlusBop,e2) => {
        (typecheck(env, e1),typecheck(env, e2)) match {
          case (StringType,t2) => StringType
          case (t1,StringType) => StringType
          case (t1,t2) if t1==t2 => t1
          case _ => throw StaticTypeError(e)
        }        
      }
      ...
      case UopExpr(NegUop,e) => {
        typecheck(env,e) match {
          case NumType => NumType
          case _ => throw StaticTypeError(e)  
        }
      }
      case UopExpr(NotUop,e) => {
        typecheck(env,e) match {
  ...
  def eval(env : Environment, e : Expr) : Value = {
    e match {
     ...
     // go through e1 and then replace all known variables at time... 
     //    ...LambdaExpr is declared
     case LambdaExpr(name : Option[String], (x : String, t)::Nil, e1 : Expr, rt) => {
        ClosureVal(if (env._1.size == 0 ) Map() else env._1(0),LambdaExpr(name,List((x,t)),e1,rt))
      }

      case CallExpr(e1 : Expr, (e2 : Expr)::Nil) => {
        val v1 = eval(env,e1) 
        v1 match {
          case ClosureVal(environment, LambdaExpr(name, List((x, t)), ex, rt)) => {
            val new_env1 = pushEnvironment((List(environment), env._2), name.get, (Immutable, v1))
