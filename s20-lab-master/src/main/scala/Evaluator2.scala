...
object Evaluator2 {
  ...
  def eval(e : Expr) : Option[Value] = {
    e match {
      ...
      case ValueExpr(NumVal(e)) => Some(NumVal(e))
      ...
      case BopExpr(e1,PlusBop,e2) => {
        (eval(e1),eval(e2)) match {
          case (Some(NumVal(e1)), Some(NumVal(e2))) => Some(NumVal(e1+e2))
          case _ => None
        }
      }
      ...
      case BopExpr(e1,EqBop,e2) => {
        (eval(e1), eval(e2)) match {
          case (Some(NumVal(e1)), Some(NumVal(e2))) => Some(BoolVal(e1 == e2))
          case (Some(BoolVal(b1)), Some(BoolVal(b2))) => Some(BoolVal(b1 == b2))
          case _ => None
        }
      }
      ...
      case UopExpr(NotUop,e1) => {
        eval(e1) match {
          //case (Some(NumVal(e1))) => Some(NumVal(!e1))
    ...
    case UopExpr(NegUop, e1) => {
      eval(e1) match {
        case (Some(NumVal(e1))) => Some(NumVal((-1)*(e1)))
        case _ => None
      }
    }
