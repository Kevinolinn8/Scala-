...
class Evaluator5Test extends FlatSpec {
  ...
  it should "handle static typing" in {
    /*const x = 5;
     ...
     const x = 7;
     f(6)*/
    val e1 = LetExpr(Immutable, "x", ve(5f), LetExpr(Immutable, "f", LambdaExpr(Some("f"), List(("y",Some(NumType))), BopExpr(VarExpr("x"),PlusBop,VarExpr("y")), None), LetExpr(Immutable, "x", ve(7f), CallExpr(VarExpr("f"), List(ve(6f))))))    //assert(Evaluator5.eval(Map(), e1) === NumVal(13f)) // dynamic typing
